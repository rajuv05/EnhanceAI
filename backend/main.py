...
async def create_processing_task(
    ...
    try:
        enhanced_filename = f"proc_{os.path.basename(file_path)}"
        enhanced_path = os.path.join("uploads/enhanced", enhanced_filename)
        
        if file_type == "image":
            output_path = image_processor.process(file_path, enhanced_path, tool)
        else:
            output_path = video_processor.process(file_path, enhanced_path, tool)
            
        db_task.status = models.TaskStatus.COMPLETED
        db_task.enhanced_path = output_path
